...
int main(int argc, char **argv)
{
    ...
    if (argc == 1)
    {
        std::cout << "Please provide the graph file path and grammar file path. For example, ./topo-driven <graph_file> <grammar_file>" << endl;
        return 0;
    }
    else if (argc == 2)
    {
        std::cout << "Please provide the grammar file path. For example, ./topo-driven <graph_file> <grammar_file>" << endl;
        return 0;
    }
    ...
    uint newEdgeSuperOne = 0;
    uint newEdgeSuperTwo = 0;
    bool finishSuperSteps = true;
    uint superSteps = 0;

    double totalTimeListConvert = 0.0;

    do
    {
        superSteps++;

        std::chrono::time_point<std::chrono::steady_clock> start_stepOut, finish_stepOut;
        start_stepOut = std::chrono::steady_clock::now();
        ...
        if (superSteps > 1)
        {
            ...
            for (uint g = 0; g < grammar.labelSize; g++)
            {
                for (uint i = 0; i < num_nodes; i++)
                {
                    edgeVecs[g][i].vertexList.clear();
                    edgeVecs[g][i].vertexList.shrink_to_fit();
                    edgeVecs[g][i].NEW_END = 0;
                    edgeVecs[g][i].OLD_END = 0;
                }
            }
            ...
            for (uint g = 0; g < grammar.labelSize; g++)
            {
                for (uint i = 0; i < num_nodes; i++)
                {
                    // clear the edgeVecs first
                    uint inSize = inEdgeVecs[g][i].vertexList.size();
                    for (uint j = 0; j < inSize; j++)
                    {
                        uint inNbr = inEdgeVecs[g][i].vertexList[j];
                        edgeVecs[g][inNbr].vertexList.push_back(i);
                    }
                }
        ...
        finish_stepOut = std::chrono::steady_clock::now();
        std::chrono::duration<double> elapsed_seconds_stepOut = finish_stepOut - start_stepOut;

        std::cout << "#Time in Step Out List Convert: " << elapsed_seconds_stepOut.count() << endl;

        totalTimeListConvert += elapsed_seconds_stepOut.count();

        finishSuperSteps = true;
        do
        {
            finished = true;
            itr++;
            ...
            std::cout << "SUPER STEP# 1: Iteration number " << itr << endl;
        ...
        std::chrono::time_point<std::chrono::steady_clock> start_step2, finish_step2;
        start_step2 = std::chrono::steady_clock::now();
        ...
        for (uint g = 0; g < grammar.labelSize; g++)
        {
            for (uint i = 0; i < num_nodes; i++)
            {
                inEdgeVecs[g][i].vertexList.clear();
                inEdgeVecs[g][i].vertexList.shrink_to_fit();

                inEdgeVecs[g][i].NEW_END = 0;
                inEdgeVecs[g][i].OLD_END = 0;
            }
        }
        ...
        for (uint g = 0; g < grammar.labelSize; g++)
        {
            ...
            for (uint i = 0; i < num_nodes; i++)
            {
                uint outSize = edgeVecs[g][i].vertexList.size();
                for (uint j = 0; j < outSize; j++)
                {
                    uint outNbr = edgeVecs[g][i].vertexList[j];
                    inEdgeVecs[g][outNbr].vertexList.push_back(i);
                }
            }
        ...
        finish_step2 = std::chrono::steady_clock::now();
        std::chrono::duration<double> elapsed_seconds_step2 = finish_step2 - start_step2;

        std::cout << "#Time in Step In List Convert: " << elapsed_seconds_step2.count() << endl;

        totalTimeListConvert += elapsed_seconds_step2.count();

        bool noNewEdgeStep3 = true;
        ...
        do
        {
                ...
                for (uint i = 0; i < num_nodes; i++)
                {
                    ...
                    for (uint j = START_NEW_IN; j < END_NEW_IN; j++)
                    {
                        ...
                        for (uint m = 0; m < g3SizeRight; m++)
                        {
                            ...
                            uint START_OLD_IN = 0;
                            uint END_OLD_IN = inEdgeVecs[C][inNbr].NEW_END;

                            for (uint h = START_OLD_IN; h < END_OLD_IN; h++)
                            {
                                uint inNbrOut = inEdgeVecs[C][inNbr].vertexList[h];

                                // calcCnt++;
                                if (hashset[inNbrOut][A].find(i) == hashset[inNbrOut][A].end())
                                {
                                    finished = false;
                                    hashset[inNbrOut][A].insert(i);
                                    // edgeVecs[A][inNbrOut].vertexList.push_back(i);
                                    inEdgeVecs[A][i].vertexList.push_back(inNbrOut);
            ...
            std::cout << "SUPER STEP# 2: Iteration number " << itr << endl;

            if (finished == false)
            {
                noNewEdgeStep3 = false;
            }

        } while (!finished);

        finishSuperSteps = noNewEdgeStep3;
    } while (!finishSuperSteps);
    ...
    std::cout << "# STEP 1 NEW EDGE: " << newEdgeSuperOne << endl;
    std::cout << "# STEP 2 NEW EDGE: " << newEdgeSuperTwo << endl;
}
