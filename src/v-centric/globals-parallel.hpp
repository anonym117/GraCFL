...
ull countEdge(vector<vector<unordered_set<ull>>> &hashset, uint hashsetSize, uint grammarLabelSize, uint flag)
{
  ull size = 0;
  for (uint i = 0; i < grammarLabelSize; i++)
  {
    for (uint j = 0; j < hashsetSize; j++)
    {
      size += hashset[i][j].size();
    }
  }
