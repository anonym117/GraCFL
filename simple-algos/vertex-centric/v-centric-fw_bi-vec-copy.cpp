	...
	do
	{
		...
		for (uint i = 0; i < num_nodes; i++)
		{
			...
			for (uint j = 0; j < edgeVecs[i][NEW].size(); j++)
			{
				...
				for (uint g = 0; g < grammar.grammar2.size(); g++)
				{
					...
					if (nbr.label == grammar.grammar2[g][1])
					{
						if (hashset[i].find(COMBINE(nbr.end, grammar.grammar2[g][0])) == hashset[i].end())
						{
							...
							inEdgeVecs[nbr.end][FUTURE].push_back(OutEdge(i, grammar.grammar2[g][0]));
				...
				for (uint h = 0; h < edgeVecs[nbr.end][NEW].size(); h++)
				{
					OutEdge outNbr = edgeVecs[nbr.end][NEW][h];
					for (uint g = 0; g < grammar.grammar3.size(); g++)
					{
						calcCnt++;
						if (nbr.label == grammar.grammar3[g][1] && outNbr.label == grammar.grammar3[g][2])
						{
							if (hashset[i].find(COMBINE(outNbr.end, grammar.grammar3[g][0])) == hashset[i].end())
							{
								hashset[i].insert(COMBINE(outNbr.end, grammar.grammar3[g][0]));
								// inHashset[outNbr.end].insert(COMBINE(i, grammar.grammar3[g][0]));
								edgeVecs[i][FUTURE].push_back(OutEdge(outNbr.end, grammar.grammar3[g][0]));
								inEdgeVecs[outNbr.end][FUTURE].push_back(OutEdge(i, grammar.grammar3[g][0]));
							}
						}
					}
				}

				for (uint h = 0; h < edgeVecs[nbr.end][OLD].size(); h++)
				{
					OutEdge outNbr = edgeVecs[nbr.end][OLD][h];
					for (uint g = 0; g < grammar.grammar3.size(); g++)
					{
						calcCnt++;
						if (nbr.label == grammar.grammar3[g][1] && outNbr.label == grammar.grammar3[g][2])
				...
				// A = CB; B = first edge, C = incoming neighbor of B
				for (uint h = 0; h < inEdgeVecs[i][OLD].size(); h++)
				{
					...
					{
						calcCnt++;
						if (inNbr.label == grammar.grammar3[g][1] && nbr.label == grammar.grammar3[g][2])
						{
							if (hashset[inNbr.end].find(COMBINE(nbr.end, grammar.grammar3[g][0])) == hashset[inNbr.end].end())
							{
								hashset[inNbr.end].insert(COMBINE(nbr.end, grammar.grammar3[g][0]));
								// inHashset[nbr.end].insert(COMBINE(inNbr.end, grammar.grammar3[g][0]));
								edgeVecs[inNbr.end][FUTURE].push_back(OutEdge(nbr.end, grammar.grammar3[g][0]));
								inEdgeVecs[nbr.end][FUTURE].push_back(OutEdge(inNbr.end, grammar.grammar3[g][0]));
							}
						}
					}
				}
