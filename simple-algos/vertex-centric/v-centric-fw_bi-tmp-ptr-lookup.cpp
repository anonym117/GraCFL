	...
	do
	{
		...
		for (uint i = 0; i < num_nodes; i++)
		{
			...
			for (uint j = START_NEW; j < END_NEW; j++)
			{
				...
				{
					calcCnt++;
					if (hashset[i][leftLabels[g]].find(nbr.end) == hashset[i][leftLabels[g]].end())
					{
						finished = false;
						hashset[i][leftLabels[g]].insert(nbr.end);
				...
				for (uint h = START_OLD_OUT; h < END_NEW_OUT; h++)
				{
					...
					{
						calcCnt++;
						if (hashset[i][leftLabels[g]].find(outNbr.end) == hashset[i][leftLabels[g]].end())
						{
							finished = false;
							hashset[i][leftLabels[g]].insert(outNbr.end);
				...
				for (uint h = START_OLD_IN; h < END_NEW_IN; h++)
				{
					...
					{
						calcCnt++;
						if (hashset[inNbr.end][leftLabels[g]].find(nbr.end) == hashset[inNbr.end][leftLabels[g]].end())
						{
							finished = false;
							hashset[inNbr.end][leftLabels[g]].insert(nbr.end);
