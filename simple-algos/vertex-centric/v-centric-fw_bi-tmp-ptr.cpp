				...
				for (uint g = 0; g < grammar.grammar2.size(); g++)
				{
					...
					if (nbr.label == grammar.grammar2[g][1])
					{
						if (hashset[i].find(COMBINE(nbr.end, grammar.grammar2[g][0])) == hashset[i].end())
						{
							...
							edgeVecs[i].vertexList.push_back(OutEdge(nbr.end, grammar.grammar2[g][0]));
							inEdgeVecs[nbr.end].vertexList.push_back(OutEdge(i, grammar.grammar2[g][0]));
					...
					for (uint g = 0; g < grammar.grammar3.size(); g++)
					{
						...
						if (nbr.label == grammar.grammar3[g][1] && outNbr.label == grammar.grammar3[g][2])
						{
							if (hashset[i].find(COMBINE(outNbr.end, grammar.grammar3[g][0])) == hashset[i].end())
							{
								...
								edgeVecs[i].vertexList.push_back(OutEdge(outNbr.end, grammar.grammar3[g][0]));
								inEdgeVecs[outNbr.end].vertexList.push_back(OutEdge(i, grammar.grammar3[g][0]));
								// No need to update the pointers. Because the FUTURE_START starts from the
								// NEW_END, and NEW_END is already updated.
							}
						}
					...
					for (uint g = 0; g < grammar.grammar3.size(); g++)
					{
						...
						if (inNbr.label == grammar.grammar3[g][1] && nbr.label == grammar.grammar3[g][2])
						{
							if (hashset[inNbr.end].find(COMBINE(nbr.end, grammar.grammar3[g][0])) == hashset[inNbr.end].end())
							{
								...
								edgeVecs[inNbr.end].vertexList.push_back(OutEdge(nbr.end, grammar.grammar3[g][0]));
								inEdgeVecs[nbr.end].vertexList.push_back(OutEdge(inNbr.end, grammar.grammar3[g][0]));
								// No need to update the pointers. Because the FUTURE_START starts from the
								// NEW_END, and NEW_END is already updated.
							}
						}
